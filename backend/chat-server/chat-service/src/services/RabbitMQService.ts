...
class RabbitMQService {
    ...
    async init() {
        ...
        await this.channel.assertQueue(config.queue.user_ststus_queue);
        this.channel.consume(config.queue.user_ststus_queue, async (msg) => {
            if (msg) {
                ...
                if (event.eventType === "USER_LOGGED_IN") {
                    ...
                    //await
                    await this.notifyDeliveredMessages(event.userId);
