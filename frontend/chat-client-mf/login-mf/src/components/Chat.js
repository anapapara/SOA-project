...
const LoggedUserLabel = lazy(() => import("chat_mf/LoggedUserLabel"));
  ...
  const connectToWebSocket = (loggedEmail) => {
    ...
    socket.onmessage = async (event) => {
      ...
      if (data.type === "NOTIFY") { //NEW_MESSAGE
        ...
        console.log(`received notify; selcted id: ${selectedUserRef.current}`)


        if (selectedUserRef.current === data.senderId) {
          ...
          console.log(`received response from unread counts: ${unreadResponse.data.data}`)
        ...
        // setTimeout(() => {   alert(`New message from ${data.senderName}`);  }, 100);
